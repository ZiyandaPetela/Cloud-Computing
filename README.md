...
*  With IaaS, you’re essentially renting the hardware in a cloud datacenter, but what you do with that
...
* Azure provides artificial intelligence (AI) and machine-learning (ML) services that can naturally communicate with your users through vision, hearing, and speech.
...
* The credit card information is used for identity verification only. You won't be charged for any services until you upgrade to a paid subscription.
...
*  the sandbox is the preferred method to use because it allows you to create and test Azure resources at no cost to you.
...
*   ![image](https://github.com/user-attachments/assets/f3e54fb0-5d07-4ac4-933d-2546e45ea434)
...
*   While a resource group can contain many resources, a single resource can only be in one resource group at a time. Some resources may be moved between resource groups, but when you move a resource to a new group, it will no longer be associated with the former group.
...
* zure management groups provide a level of scope above subscriptions. You organize subscriptions into containers called management groups and apply governance conditions to the management groups. 
...
* ![image](https://github.com/user-attachments/assets/3226687f-6c18-4c66-bc7c-30df7a955dc8)
...
* different services that should be available with Azure Storage.
* Azure Storage provides a variety of services designed to store and manage different types of data. Here are the main services available:
* Blob Storage:
...
* Page blobs: Store random access files up to 8 TB in size, used for virtual hard disk files.
* File Storage:
* Azure Files: Offers fully managed file shares in the cloud that are accessible via the Server Message Block (SMB) protocol or Network File System (NFS) protocol.
* Useful for migrating existing applications that depend on file shares to Azure without changing their code.
* Queue Storage:
* Azure Queue Storage: Provides a message queueing service for storing large numbers of messages that can be accessed from anywhere via authenticated calls using HTTP or HTTPS.
Ideal for decoupling and scaling components of cloud applications.
* Table Storage:
* Azure Table Storage: Offers NoSQL key-attribute data store, ideal for storing structured, non-relational data.
* It is highly scalable and can handle large amounts of data efficiently.
* Disk Storage:
* Azure Disk Storage: Provides persistent, managed disks for Azure VMs.
* Types of disks include:
* Premium SSD: High-performance, low-latency disk storage.
* Standard SSD: Cost-effective, SSD-based disk storage.
* Standard HDD: Economical, HDD-based disk storage.
* Data Lake Storage:
* Azure Data Lake Storage: Combines the scalability and cost benefits of an object storage service with the reliability and performance of a data lake built for enterprise data workloads.
* It is built on Azure Blob Storage and optimized for big data analytics.
Each of these services is designed to handle specific types of data storage needs, offering flexibility and scalability for different use cases in cloud applications.
